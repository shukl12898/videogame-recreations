...
void PacManMove::Update(float deltaTime)
{
	...
	if (!Math::NearZero(mMoveDir.x) && !Math::NearZero(mInput.x))
	{
		mMoveDir.x = mInput.x;
	}
	else if (!Math::NearZero(mMoveDir.y) && !Math::NearZero(mInput.y))
	{
		mMoveDir.y = mInput.y;
	}
...
void PacManMove::HandleNodeIntersect(PathNode* p)
{
	...
	if (!Math::NearZero(mMoveDir.x) && !Math::NearZero(mInput.y))
	{
	...
	else if (!Math::NearZero(mMoveDir.y) && !Math::NearZero(mInput.x))
	{
